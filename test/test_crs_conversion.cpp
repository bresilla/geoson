...
TEST_CASE("CRS conversion during output") {
        ...
        {"features", {
            {
                ...
                {"geometry", {
                    {"type", "Point"},
                    {"coordinates", {5.1, 52.1, 105.0}}  // lon, lat, alt in WGS
                }},
    ...
    SUBCASE("Parse and verify internal representation") {
        auto fc = geoson::ReadFeatureCollection("test_crs_input.geojson");
    ...
    SUBCASE("Output in different CRS formats") {
        ...
        geoson::WriteFeatureCollection(fc, "test_output_wgs.geojson", geoson::CRS::WGS);
        auto fc_wgs = geoson::ReadFeatureCollection("test_output_wgs.geojson");
        CHECK(fc_wgs.crs == geoson::CRS::WGS);
        
        // Test output in ENU format
        geoson::WriteFeatureCollection(fc, "test_output_enu.geojson", geoson::CRS::ENU);
        auto fc_enu = geoson::ReadFeatureCollection("test_output_enu.geojson");
