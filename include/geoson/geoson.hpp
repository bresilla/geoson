...
namespace geoson {
    ...
    class GeoJSONObject {
      public:
        enum class ObjectType { Geometry, Feature, FeatureCollection };
    ...
    class Geometry : public GeoJSONObject {
      public:
        enum class Type { Point, MultiPoint, LineString, MultiLineString, Polygon, MultiPolygon, GeometryCollection };
    ...
    class Feature : public GeoJSONObject {
      public:
        Feature(std::shared_ptr<Geometry> geometry, const std::map<std::string, Value> &properties,
    ...
    class FeatureCollection : public GeoJSONObject {
      public:
        FeatureCollection(const std::vector<std::shared_ptr<Feature>> &features) : _features(features) {}
    ...
    // -- Helper #1: geometry → string
    inline std::string to_string(const std::shared_ptr<Geometry> &geometry, bool errs = false) {
        using Type = Geometry::Type;
        std::ostringstream out;

        switch (geometry->type()) {
        case Type::Point: {
            auto p = std::static_pointer_cast<Point>(geometry);
            auto c = p->coordinate();
            out << "Point: (" << c.x << ", " << c.y;
            if (c.hasZ()) out << ", " << c.z;
            out << ")";
        ...
        case Type::MultiPoint: {
            auto mp = std::static_pointer_cast<MultiPoint>(geometry);
            out << "MultiPoint:";
            for (auto const &c : mp->points()) {
                out << "\n  (" << c.x << ", " << c.y;
                if (c.hasZ()) out << ", " << c.z;
                out << ")";
            }
        ...
        case Type::LineString: {
            auto ls = std::static_pointer_cast<LineString>(geometry);
            out << "LineString:";
            for (auto const &c : ls->points()) {
                out << "\n  (" << c.x << ", " << c.y;
                if (c.hasZ()) out << ", " << c.z;
                out << ")";
            }
        ...
        case Type::MultiLineString: {
            auto mls = std::static_pointer_cast<MultiLineString>(geometry);
            out << "MultiLineString:";
            int idx = 0;
            for (auto const &line : mls->lines()) {
                out << "\n  Line " << idx++ << ":";
        ...
        case Type::Polygon: {
            auto poly = std::static_pointer_cast<Polygon>(geometry);
            out << "Polygon:";
            int r = 0;
        ...
        case Type::MultiPolygon: {
            auto mpoly = std::static_pointer_cast<MultiPolygon>(geometry);
            out << "MultiPolygon:";
            int pidx = 0;
            for (auto const &poly : mpoly->polygons()) {
                out << "\n  Polygon " << pidx++ << ":";
                int ridx = 0;
        ...
        case Type::GeometryCollection: {
            auto coll = std::static_pointer_cast<GeometryCollection>(geometry);
            out << "GeometryCollection:";
            for (auto const &g : coll->geometries()) {
                out << "\n" << to_string(g);
            }
            break;
        }
    ...
    // -- Helper #2: GeoJSON object → string
    inline std::string to_string(const std::shared_ptr<GeoJSONObject> &obj, bool errs = false) {
        using OT = GeoJSONObject::ObjectType;
        std::ostringstream out;

        switch (obj->objectType()) {
        case OT::Geometry:
            out << to_string(std::static_pointer_cast<Geometry>(obj), errs);
            break;

        case OT::Feature: {
            auto feat = std::static_pointer_cast<Feature>(obj);
            out << "Feature:";
            if (!feat->id().isNull()) {
                out << "\n  ID: ";
                if (feat->id().isString())
                    out << feat->id().asString();
                else if (feat->id().isNumber())
                    out << feat->id().asNumber();
            }
            out << "\n  Properties:";
            for (auto const &kv : feat->properties()) {
                out << "\n    " << kv.first << ": ";
                auto &v = kv.second;
                if (v.isString())
                    out << v.asString();
            ...
            out << "\n  Geometry:\n    ";
            if (feat->geometry())
                out << to_string(feat->geometry(), errs);
            else
                out << "null";
            break;
        }

        case OT::FeatureCollection: {
            auto fc = std::static_pointer_cast<FeatureCollection>(obj);
            out << "FeatureCollection with " << fc->features().size() << " features:";
        ...
        default:
            if (errs) {
                out << "Unsupported GeoJSON object type.";
            }
        }
    ...
    // -- Now your print functions simply delegate to the above:
    inline void printGeometry(const std::shared_ptr<Geometry> &geometry) {
        std::cout << to_string(geometry) << std::endl;
    }

    inline void printGeoJSONObject(const std::shared_ptr<GeoJSONObject> &obj) {
        std::cout << to_string(obj) << std::endl;
    }

    /// Dump any GeoJSON object (geometry, feature, or collection)
    inline void writeGeoJSONFile(const std::shared_ptr<GeoJSONObject> &obj, const std::string &path) {
