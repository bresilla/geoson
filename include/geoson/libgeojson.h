...
namespace geoson {
    ...
    /** \overload */
    inline nlohmann::json Position(double lon, double lat) { return nlohmann::json::array({lon, lat}); }
    ...
    namespace detail {
         ...
         *          object
         */
        inline nlohmann::json PointCoordinates(double lon, double lat, double alt) { return Position(lon, lat, alt); }

        /** \overload */
        inline nlohmann::json PointCoordinates(double lon, double lat) { return Position(lon, lat); }

        /** Returns a GeoJSON object, with a "type" and "coordinates" object */
        template <Type T> inline nlohmann::json CoordinatesObject(nlohmann::json &&coords) {
            return nlohmann::json{{"type", TypeName<T>()}, {"coordinates", std::move(coords)}};
        }
    } // namespace detail
    ...
    inline nlohmann::json Point(double lon, double lat, double alt) {
        return detail::CoordinatesObject<Type::Point>(detail::PointCoordinates(lon, lat, alt));
    }

    /** \overload */
    inline nlohmann::json Point(double lon, double lat) {
        return detail::CoordinatesObject<Type::Point>(detail::PointCoordinates(lon, lat));
    }
    ...
    namespace detail {
        ...
        template <class F, class... Args> using invoke_result = std::invoke_result<F, Args...>;

        template <typename F, typename... Args> using is_invocable = std::is_invocable<F, Args...>;

        template <typename F, typename... Args> using is_invocable_r = std::is_invocable_r<F, Args...>;

#else

        template <class F, class... Args> using invoke_result = std::result_of<F(Args...)>;
        ...
        using IsCallbackSignature =
            typename std::enable_if<std::is_same<typename detail::invoke_result<F, Args...>::type, R>::value,
                                    bool>::type;
        ...
        template <typename Callable,
                  detail::IsCallbackSignature<Callable, void, size_t, double &, double &, double &> = true>
        ...
        /** \overload */
        template <typename Callable, detail::IsCallbackSignature<Callable, void, size_t, double &, double &> = true>
        nlohmann::json MultiPointCoordinates(size_t numPoints, Callable &&getPoint) {
            auto coords = nlohmann::json::array();
            double lon, lat;
            for (size_t i = 0; i < numPoints; i++) {
     ...
     */
    template <typename Callable> nlohmann::json MultiPoint(size_t numPoints, Callable &&getPoint) {
        static_assert(detail::is_invocable_r<void, Callable, size_t, double &, double &, double &>::value ||
                          detail::is_invocable_r<void, Callable, size_t, double &, double &>::value,
                      "Callback must either be void(size_t, double&, double&, double&) or "
                      "void(size_t, double&, double&)");
        return detail::CoordinatesObject<Type::MultiPoint>(
            detail::MultiPointCoordinates(numPoints, std::forward<Callable>(getPoint)));
    }
     ...
     */
    template <typename Callable> nlohmann::json LineString(size_t numPoints, Callable &&getPoint) {
        static_assert(detail::is_invocable_r<void, Callable, size_t, double &, double &, double &>::value ||
                          detail::is_invocable_r<void, Callable, size_t, double &, double &>::value,
                      "Callback must either be void(size_t, double&, double&, double&) or "
                      "void(size_t, double&, double&)");
        return detail::CoordinatesObject<Type::LineString>(
            detail::LineStringCoordinates(numPoints, std::forward<Callable>(getPoint)));
    }
    ...
    namespace detail {
        ...
        template <typename GetLineLength, typename GetPoint,
                  detail::IsCallbackSignature<GetPoint, void, size_t, size_t, double &, double &, double &> = true>
        nlohmann::json MultiLineStringCoordinates(size_t numLines, GetLineLength &&getLineLength, GetPoint &&getPoint) {
            auto coords = nlohmann::json::array();
            for (size_t i = 0; i < numLines; i++) {
                coords.push_back(detail::LineStringCoordinates(
                    getLineLength(i),
                    [&](size_t j, double &lon, double &lat, double &alt) { getPoint(i, j, lon, lat, alt); }));
            }
            return coords;
        }
        ...
        /* \overload */
        template <typename GetLineLength, typename GetPoint,
                  detail::IsCallbackSignature<GetPoint, void, size_t, size_t, double &, double &> = true>
        nlohmann::json MultiLineStringCoordinates(size_t numLines, GetLineLength &&getLineLength, GetPoint &&getPoint) {
            auto coords = nlohmann::json::array();
            for (size_t i = 0; i < numLines; i++) {
                coords.push_back(detail::LineStringCoordinates(
                    getLineLength(i), [&](size_t j, double &lon, double &lat) { getPoint(i, j, lon, lat); }));
            }
            return coords;
        }
    } // namespace detail
     ...
     */
    template <typename GetLineLength, typename GetPoint>
    nlohmann::json MultiLineString(size_t numLineStrings, GetLineLength &&getLineLength, GetPoint &&getPoint) {
        static_assert(detail::is_invocable_r<void, GetPoint, size_t, size_t, double &, double &, double &>::value ||
                          detail::is_invocable_r<void, GetPoint, size_t, size_t, double &, double &>::value,
                      "GetPoint callback must either be void(size_t, size_t, double&, "
                      "double&, double&) or void(size_t, size_t, double&, double&)");
        return detail::CoordinatesObject<Type::MultiLineString>(detail::MultiLineStringCoordinates(
            numLineStrings, std::forward<GetLineLength>(getLineLength), std::forward<GetPoint>(getPoint)));
    }
    ...
    namespace detail {
        ...
        inline bool IsCcw(const nlohmann::json &coords) {
            ...
            for (size_t i = 0; i < coords.size(); i++) {
                ...
                cwEdgeSum += (pt2[0].template get<double>() - pt1[0].template get<double>()) *
        ...
        nlohmann::json LinearRingCoordinates(size_t numPoints, bool ccw, GetPoint &&getPoint) {
            ...
            auto coords = LineStringCoordinates(numPoints, std::forward<GetPoint>(getPoint));
        ...
        template <typename GetRingLength, typename GetPoint,
                  detail::IsCallbackSignature<GetPoint, void, size_t, size_t, double &, double &, double &> = true>
        nlohmann::json PolygonCoordinates(size_t numRings, GetRingLength &&getRingLength, GetPoint &&getPoint) {
            nlohmann::json coords;
            for (size_t i = 0; i < numRings; i++) {
                coords.push_back(detail::LinearRingCoordinates(
                    getRingLength(i), i == 0,
                    [&](size_t j, double &lat, double &lon, double &alt) { getPoint(i, j, lat, lon, alt); }));
            }
        ...
        nlohmann::json PolygonCoordinates(size_t numRings, GetRingLength &&getRingLength, GetPoint &&getPoint) {
            ...
            for (size_t i = 0; i < numRings; i++) {
                coords.push_back(detail::LinearRingCoordinates(
                    getRingLength(i), i == 0, [&](size_t j, double &lat, double &lon) { getPoint(i, j, lat, lon); }));
            }
     ...
     */
    template <typename GetRingLength, typename GetPoint>
    nlohmann::json Polygon(size_t numRings, GetRingLength &&getRingLength, GetPoint &&getPoint) {
        static_assert(detail::is_invocable_r<void, GetPoint, size_t, size_t, double &, double &, double &>::value ||
                          detail::is_invocable_r<void, GetPoint, size_t, size_t, double &, double &>::value,
                      "GetPoint callback must either be void(size_t, size_t, double&, "
                      "double&, double&) or void(size_t, size_t, double&, double&)");
        return detail::CoordinatesObject<Type::Polygon>(detail::PolygonCoordinates(
            numRings, std::forward<GetRingLength>(getRingLength), std::forward<GetPoint>(getPoint)));
    }
    ...
    namespace detail {
         ...
         */
        template <
            typename GetNumRings, typename GetRingLength, typename GetPoint,
            detail::IsCallbackSignature<GetPoint, void, size_t, size_t, size_t, double &, double &, double &> = true>
        nlohmann::json MultiPolygonCoordinates(size_t numPolygons, GetNumRings &&getNumRings,
                                               GetRingLength &&getRingLength, GetPoint &&getPoint) {
            nlohmann::json coords;
            for (size_t i = 0; i < numPolygons; i++) {
                coords.push_back(detail::PolygonCoordinates(
                    getNumRings(i), [&](size_t ring) -> size_t { return getRingLength(i, ring); },
        ...
        template <typename GetNumRings, typename GetRingLength, typename GetPoint,
                  detail::IsCallbackSignature<GetPoint, void, size_t, size_t, size_t, double &, double &> = true>
        nlohmann::json MultiPolygonCoordinates(size_t numPolygons, GetNumRings &&getNumRings,
                                               GetRingLength &&getRingLength, GetPoint &&getPoint) {
            nlohmann::json coords;
            for (size_t i = 0; i < numPolygons; i++) {
                coords.push_back(PolygonCoordinates(
                    getNumRings(i), [&](size_t ring) -> size_t { return getRingLength(i, ring); },
                    [&](size_t ring, size_t pt, double &lon, double &lat) { getPoint(i, ring, pt, lon, lat); }));
            }
            return coords;
        }
    } // namespace detail
     ...
     */
    template <typename GetNumRings, typename GetRingLength, typename GetPoint>
    nlohmann::json MultiPolygon(size_t numPolygons, GetNumRings &&getNumRings, GetRingLength &&getRingLength,
                                GetPoint &&getPoint) {
        static_assert(
            detail::is_invocable_r<void, GetPoint, size_t, size_t, size_t, double &, double &, double &>::value ||
                detail::is_invocable_r<void, GetPoint, size_t, size_t, size_t, double &, double &>::value,
        ...
        return detail::CoordinatesObject<Type::MultiPolygon>(detail::MultiPolygonCoordinates(
            numPolygons, std::forward<GetNumRings>(getNumRings), std::forward<GetRingLength>(getRingLength),
     ...
     */
    template <typename Callable> nlohmann::json GeometryCollection(size_t numGeometries, Callable &&getGeometry) {
        nlohmann::json j;
        for (size_t i = 0; i < numGeometries; i++) {
            j.push_back(getGeometry(i));
        }

        return nlohmann::json{{"type", TypeName<Type::GeometryCollection>()}, {"geometries", j}};
    }
    ...
    inline nlohmann::json Feature(const nlohmann::json &geometry, const nlohmann::json &properties) {
        return nlohmann::json{{"type", TypeName<Type::Feature>()}, {"geometry", geometry}, {"properties", properties}};
    }
     ...
     *  \return A GeoJSON Feature object
     */
    inline nlohmann::json Feature(const std::string &id, const nlohmann::json &geometry,
    ...
    inline nlohmann::json Feature(T id, const nlohmann::json &geometry, const nlohmann::json &properties) {
        auto j = Feature(geometry, properties);
        j["id"] = id;
        return j;
    }
     ...
     *  \return A GeoJSON Feature object
     */
    template <typename Callback> inline nlohmann::json FeatureCollection(size_t numFeatures, Callback &&getFeature) {
